"""Shared utilities — retry/backoff helpers.

Lifted verbatim from redhat-community-ai-tools/code-to-docs (MIT licensed).
See plan.md "Patterns adopted from code-to-docs" — this is Pattern 2 (modular
single-responsibility files) carrying along Pattern's small utility helpers.
"""
import time
...
def retry_with_backoff(
    ...
    Args:
        ...
        default: Value to return if all retries exhausted (ignored if reraise=True)
...
def calc_backoff_delay(attempt, multiplier=3):
    """Linear backoff: (attempt + 1) * multiplier seconds."""
